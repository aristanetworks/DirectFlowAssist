...
Arista Networks, Inc.
EOS Extension: DirectFlow Assist for Check Point Firewall 
-------------------------------------------------------------------------------

Minimum EOS version: 4.14.5F or later
This extension has been test with: Open ServerR77_30 build# 152
...
--------------------------------------------------------------------------------
Check Point Firewall Specific Information:

* To send syslog messages to DFA from a CheckPoint firewall/gateway:
  - from SmartDashboard under Servers and OPSEC > Servers > Syslog
    - right click, select NEW, then add Arista switch management interface 
      IP address and port 9514

* For the services/applications to be 'assisted' (i.e. bypassed or dropped at
the switch) do the following:
- from SmartDashboard under Services select the group then right click
  select NEW then define a service name that ends with '_DFA' and set the 
  proper layer 4 port number.  For example define service name 'ssh_DFA' port 22
- then use this new service name in a policy rule and set the rule to Log 
  in the Track field

This suffix can be changed in config.py: ASSIST_WHEN_SERVICE_ID_SUFFIX = '_DFA'
