''' settings for DirectFlow Assist for PAN Firewalls - DEMO Package
'''
#
...
FIREWALL_INTERFACE_AF = 'ethernet1/3'   # use same firewall intf name on AF and HA_AF, don't include sub-intf
...
FIREWALL_INTERFACE_BF = 'ethernet1/4'   # use same firewall intf name on AF and HA_AF, don't include sub-intf
...
THREAT_DROP_DEFAULT = {
    'Type': 'THREAT',
    'Action': ['^drop$',       # match whole field, don't want to match 'random-drop'
               'drop-all-packets', 'deny', 'block-url', 'reset-.*']}
...
# DEMO PKG: Palo Alto Networks package specific settings
DEMO_CTRL_UDP_PORT = 9515  # port number must be the same in DemoRemote.py
OPEN_DEMO_CTRL_PORT_CMD = 'sudo iptables -A INPUT -p udp --dport %d -j ACCEPT' % DEMO_CTRL_UDP_PORT
CLOSE_DEMO_CTRL_PORT_CMD = 'sudo iptables -D INPUT -p udp --dport %d -j ACCEPT' % DEMO_CTRL_UDP_PORT

SSL_CA_CERTS_FILE = '/persist/sys/extensions/directflow_assist_pan_demo/rootCA2.crt'
SSL_DFA_CERT_FILE = '/persist/sys/extensions/directflow_assist_pan_demo/dfa.crt'
SSL_DFA_KEY_FILE = '/persist/sys/extensions/directflow_assist_pan_demo/dfa.key'

APP_NAME = 'DirectFlow Assist for PAN Firewalls - DEMO'
...
def get_msg_class():
    ''' runtime import '''
    from directflow_assist_pan_demo import PANSyslogMsg
